...
#ifndef CDATAFILE_HPP
#define CDATAFILE_HPP
...
namespace cdf {
  ...
  // the requested section does not already exist.
  const int AUTOCREATE_SECTIONS = 2;
  // constexpr int AUTOCREATE_SECTIONS  { 1L<<1 };

  // AUOTCREATE_KEYS
  // When set, this define causes SetValue() to create a new key, if the
  // requested key does not already exist.
  const int AUTOCREATE_KEYS = 4;
  // constexpr int AUTOCREATE_KEYS         { 1L<<2 };
  ...
  // constexpr int MAX_BUFFER_LEN          { 512 };
